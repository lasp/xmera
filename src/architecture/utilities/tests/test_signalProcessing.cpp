...
double calculateStandardDeviation(const std::vector<double>& input) {
    ...
    double standardDeviation = std::accumulate(input.begin(), input.end(), 0.0, [mean](int previousResult, int value) {
...
TEST_P(GeneralPerformance, lowPassFilterProperties) {
    ...
    auto residualsWithoutTransient = std::vector<Eigen::Vector3d>(residuals.begin() + 100, residuals.end());
...
INSTANTIATE_TEST_SUITE_P(LowPassFilter,
                         GeneralPerformance,
                         /* Testing parameters:
                          * -timeStep : filter time step
                          * -omegaCutOff : rad/s cut off of the filter
                          * */
                         ::testing::Values(std::make_tuple(0.01, 2.5),
