...
void LowPassFilter::processMeasurement(const Eigen::Vector3d& measurement) {
    ...
    this->currentState = 1 / (2 + omegaStep) *
                         (this->currentState * (2 - omegaStep) + omegaStep * (measurement + this->currentMeasurement));
 ...
 */
Eigen::Vector3d LowPassFilter::getCurrentState() const { return this->currentState; };

/**
 * Set the time step used in the low pass filter
 * @param filterStepSeconds double
 */
void LowPassFilter::setFilterStep(const double filterStepSeconds) { this->filterStep = filterStepSeconds; };

/**
 * Get the time step used in the low pass filter
 * @return filterStepSeconds double
 */
double LowPassFilter::getFilterStep() const { return this->filterStep; };

/**
 * Set the cut off value (norm of the measurements) in the low pass
 * @param cutOffValue double
 */
void LowPassFilter::setFilterCutoff(const double cutOffValue) { this->filterCutOff = cutOffValue; };

/**
 * Get the cut off value (norm of the measurements) in the low pass
 * @return cutOffValue double
 */
double LowPassFilter::getFilterCutoff() const { return this->filterCutOff; };
