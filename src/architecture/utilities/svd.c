...
int svdcmp(double* mx, size_t dim1, size_t dim2, double* w, double* v) {
    ...
    for (i = 0; i < dim2; i++) {
        ...
        g = s = scale = 0.0;
        if (i < dim1) {
            for (k = i; k < dim1; k++) scale += fabs(mx[MXINDEX(dim2, k, i)]);
            if (scale) {
                for (k = i; k < dim1; k++) {
                ...
                for (j = l; j < dim2; j++) {
                    for (s = 0.0, k = i; k < dim1; k++) s += mx[MXINDEX(dim2, k, i)] * mx[MXINDEX(dim2, k, j)];
                    f = s / h;
                    for (k = i; k < dim1; k++) mx[MXINDEX(dim2, k, j)] += f * mx[MXINDEX(dim2, k, i)];
                }
                for (k = i; k < dim1; k++) mx[MXINDEX(dim2, k, i)] *= scale;
            }
        }
        w[i] = scale * g;
        g = s = scale = 0.0;
        if (i < dim1 && i != dim2 - 1) {
            for (k = l; k < dim2; k++) scale += fabs(mx[MXINDEX(dim2, i, k)]);
            if (scale) {
                for (k = l; k < dim2; k++) {
                ...
                for (k = l; k < dim2; k++) rv1[k] = mx[MXINDEX(dim2, i, k)] / h;
                for (j = l; j < dim1; j++) {
                    for (s = 0.0, k = l; k < dim2; k++) s += mx[MXINDEX(dim2, j, k)] * mx[MXINDEX(dim2, i, k)];
                    for (k = l; k < dim2; k++) mx[MXINDEX(dim2, j, k)] += s * rv1[k];
                }
                for (k = l; k < dim2; k++) mx[MXINDEX(dim2, i, k)] *= scale;
            }
        }
    ...
    for (i = (int)dim2 - 1; i >= 0; i--) {
        if (i < dim2 - 1) {
            if (g) {
                ...
                for (j = l; j < dim2; j++) {
                    for (s = 0.0, k = l; k < dim2; k++) s += mx[MXINDEX(dim2, i, k)] * v[MXINDEX(dim2, k, j)];
                    for (k = l; k < dim2; k++) v[MXINDEX(dim2, k, j)] += s * v[MXINDEX(dim2, k, i)];
                }
            }
            for (j = l; j < dim2; j++) v[MXINDEX(dim2, i, j)] = v[MXINDEX(dim2, j, i)] = 0.0;
        }
    ...
    for (i = IMIN((int)dim1, (int)dim2) - 1; i >= 0; i--) {
        l = i + 1;
        g = w[i];
        for (j = l; j < dim2; j++) mx[MXINDEX(dim2, i, j)] = 0.0;
        if (g) {
            g = 1.0 / g;
            for (j = l; j < dim2; j++) {
                for (s = 0.0, k = l; k < dim1; k++) s += mx[MXINDEX(dim2, k, i)] * mx[MXINDEX(dim2, k, j)];
                f = (s / mx[MXINDEX(dim2, i, i)]) * g;
                for (k = i; k < dim1; k++) mx[MXINDEX(dim2, k, j)] += f * mx[MXINDEX(dim2, k, i)];
            }
            for (j = i; j < dim1; j++) mx[MXINDEX(dim2, j, i)] *= g;
        } else
            for (j = i; j < dim1; j++) mx[MXINDEX(dim2, j, i)] = 0.0;
        ++mx[MXINDEX(dim2, i, i)];
    }
    ...
    for (k = (int)dim2 - 1; k >= 0; k--) {
        for (its = 0; its < 30; its++) {
            ...
            if (l == k) {
                if (z < 0.0) {
                    w[k] = -z;
                    for (j = 0; j < dim2; j++) v[MXINDEX(dim2, j, k)] = -v[MXINDEX(dim2, j, k)];
                }
                break;
            }
...
void solveSVD(double* mx, size_t dim1, size_t dim2, double* x, double* b, double minSV) {
    ...
    // condition wInvDiag
    for (j = 0; j < dim2; j++) {
        if (w[j] >= minSV) wInvDiag[MXINDEX(dim2, j, j)] = 1.0 / w[j];
    }
