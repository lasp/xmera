...
void GaussMarkov::computeNextState() {
    ...
    if ((this->propMatrix.size() != this->noiseMatrix.size()) ||
        ...
        bskLogger.bskLog(BSK_ERROR,
                         "For the Gauss Markov model, you HAVE, and I mean HAVE, to have your propagate and noise "
                         "matrices be same size and that size is your number of states squared.  I quit.");
        return;
    }
    if ((uint64_t)this->stateBounds.size() != this->numStates) {
        bskLogger.bskLog(BSK_ERROR,
                         "For the Gauss Markov model, you HAVE, and I mean HAVE, to have your walk bounds length equal "
                         "to your number of states. I quit.");
        return;
    }
    ...
    for (i = 0; i < this->numStates; i++) {
        ...
        if (this->stateBounds[i] > 0.0) {
            double stateCalc = fabs(this->currentState[i]) > this->stateBounds[i] * 1E-10 ? fabs(this->currentState[i])
                                                                                          : this->stateBounds[i];
