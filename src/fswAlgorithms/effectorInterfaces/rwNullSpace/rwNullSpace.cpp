...
void RwNullSpace::reset(uint64_t callTime) {
    ...
    if (!this->rwConfigInMsg.isLinked()) { throw std::invalid_argument("rwNullSpace.rwConfigInMsg wasn't connected."); }
    if (!this->rwMotorTorqueInMsg.isLinked()) {
        throw std::invalid_argument("rwNullSpace.rwMotorTorqueInMsg wasn't connected.");
    }
    if (!this->rwSpeedsInMsg.isLinked()) { throw std::invalid_argument("rwNullSpace.rwSpeedsInMsg wasn't connected."); }
 ...
 */
void RwNullSpace::setOmegaGain(double const gain) {
    this->algorithm.setOmegaGain(gain);
}
 ...
 */
double RwNullSpace::getOmegaGain() const {
    return this->algorithm.getOmegaGain();
}
