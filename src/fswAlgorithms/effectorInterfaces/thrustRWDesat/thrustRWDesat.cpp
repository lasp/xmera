...
void ThrustRWDesat::updateState(uint64_t callTime) {
    ...
    for (i = 0; i < this->numRWAs; i++) {
        v3Scale(rwSpeeds.wheelSpeeds[i], &(this->rwAlignMap[i * 3]), singleSpeedVec);
    ...
    if (v3Dot(this->currDMDir, observedSpeedVec) <= this->DMThresh) {
        for (i = 0; i < this->numThrusters; i++) {
            fireValue = v3Dot(observedSpeedVec, &(this->thrTorqueMap[i * 3]));
            currentMatch = v3Dot(this->accumulatedImp, &(this->thrAlignMap[i * 3]));
    ...
    */
    ...
    bestMatch = 0.0;
    for (i = 0; i < this->numThrusters; i++) {
        fireValue = v3Dot(this->currDMDir, &(this->thrTorqueMap[i * 3]));
        currentMatch = v3Dot(this->accumulatedImp, &(this->thrAlignMap[i * 3]));
        if (fireValue - currentMatch > bestMatch && fireValue > 0.0) {
            selectedThruster = i;
            bestMatch = fireValue - currentMatch;
    ...
    if (bestMatch > 0.0) {
        ...
        v3Add(this->accumulatedImp, singleSpeedVec, this->accumulatedImp);
    }
