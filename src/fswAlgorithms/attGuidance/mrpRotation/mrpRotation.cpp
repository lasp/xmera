...
void MrpRotation::reset(uint64_t callTime) {
    ...
    if (!this->attRefInMsg.isLinked()) { throw std::invalid_argument("mrpRotation.attRefInMsg wasn't connected."); }

    // Check if optional input messages are linked
    if (this->desiredAttInMsg.isLinked()) { this->algorithm.enableDynamicReference(); }
...
void MrpRotation::updateState(uint64_t callTime) {
    ...
    if (this->algorithm.isDynamicReferenceEnabled()) { attStates = this->desiredAttInMsg(); }
...
*/
void MrpRotation::setSigmaRR0(Eigen::Vector3d const &sigma) {
...
*/
Eigen::Vector3d const MrpRotation::getSigmaRR0() const {
    return this->algorithm.getSigmaRR0();
}
...
*/
void MrpRotation::setOmegaRR0(Eigen::Vector3d const &omega) {
...
/*! Getter method for the angular velocity vector relative to input reference
 @return const Eigen::Vector3d
*/
Eigen::Vector3d const MrpRotation::getOmegaRR0() const {
    return this->algorithm.getOmegaRR0();
}
