...
#ifndef _TWO_REF_POINT_H_
#define _TWO_REF_POINT_H_
 ...
 */
class TwoRefPoint : public SysModel {
public:
    void reset(uint64_t callTime) override;
    void updateState(uint64_t callTime) override;
    void parseInputMessages();
    void computeTwoRefPoint(uint64_t callTime);
    /* Declare module private variables */
    double singularityThresh;       //!< (r) Threshold for when to fix constraint axis*/
    double R_P1B_N[3];              //!< [m] refrence 1 position vector relative to inertial frame, in N-frame components
    double R_P2B_N[3];              //!< [m] refrence 2 position vector relative to inertial frame, in N-frame components
