...
static RegionOfInterestMsgPayload regionAt(RegionsIdentifiedMsgPayload const &msg, uint32_t k) {
    return {
        .timeTag = msg.timeTag[k],
...
void RegionsOfInterestPrune::updateState(uint64_t callTime) {
    ...
    FpgaRowColSumMsgPayload const rcMsg = this->rowColSumInMsg();
    auto const* rowSums = static_cast<uint16_t const*>(rcMsg.rowSumPointer);
    auto const* colSums = static_cast<uint16_t const*>(rcMsg.colSumPointer);

    RoiCandidates const candidates = this->algorithm.update(rowSums, rcMsg.numRows, colSums, rcMsg.numCols);

    this->lastRegionsOutput = {};
    this->numPublished = std::min(candidates.numCandidates, static_cast<uint32_t>(MAX_NUMBER_REGIONS));
    double const timeTagSec = static_cast<double>(callTime) * NANO2SEC;
    for (uint32_t k = 0; k < this->numPublished; ++k) {
        auto const &cand = candidates.candidates[k];
...
*/
cv::Mat RegionsOfInterestPrune::buildBackground(FpgaRowColSumMsgPayload const &rcMsg) {
    auto const H = static_cast<int>(rcMsg.numRows);
    auto const W = static_cast<int>(rcMsg.numCols);

    if (this->threshImageInMsg.isLinked()) {
        FpgaThreshImageMsgPayload const thMsg = this->threshImageInMsg();
        auto const* bits = reinterpret_cast<uint8_t const*>(thMsg.imagePointer);
...
*/
void RegionsOfInterestPrune::drawRegion(
    cv::Mat &vis,
    RegionOfInterestMsgPayload const &reg,
    cv::Scalar const &color,
    int thickness,
    std::string const &label
) {
    int const x = reg.centerX - reg.width / 2;
    int const y = reg.centerY - reg.height / 2;
...
void RegionsOfInterestPrune::saveVisualization(FpgaRowColSumMsgPayload const &rcMsg) {
    if (rcMsg.numRows == 0 || rcMsg.numCols == 0) { return; }

    cv::Mat vis = buildBackground(rcMsg);
    if (vis.empty()) { return; }

    uint32_t const nDraw = this->numPublished;

    // All published regions: thin cyan outline for context.
    for (uint32_t k = 0; k < nDraw; ++k) {
        int const x = this->lastRegionsOutput.centerX[k] - this->lastRegionsOutput.width[k] / 2;
        int const y = this->lastRegionsOutput.centerY[k] - this->lastRegionsOutput.height[k] / 2;
        cv::rectangle(
            vis,
            cv::Point(x, y),
    ...
    static cv::Scalar const kRed(0, 0, 255);
    static cv::Scalar const kBlue(255, 0, 0);
    if (nDraw >= 1) {
        auto const r1 = regionAt(this->lastRegionsOutput, 0);
        drawRegion(vis, r1, kRed, 2, "R1 (" + std::to_string(r1.numberOfPixels) + ")");
    }
    if (nDraw >= 2) {
        auto const r2 = regionAt(this->lastRegionsOutput, 1);
