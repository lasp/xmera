...
void MrpPD::reset(uint64_t callTime) {
    if (!this->guidInMsg.isLinked()) { throw std::invalid_argument("mrpPD.guidInMsg wasn't connected."); }
    if (!this->vehConfigInMsg.isLinked()) { throw std::invalid_argument("mrpPD.vehConfigInMsg wasn't connected."); }
...
void MrpPD::updateState(uint64_t callTime) {
    ...
    if (this->guidInMsg.isWritten()) { localGuidInMsg = this->guidInMsg(); }
...
*/
void MrpPD::setDerivativeGainP(double P) {
...
/*! Getter method for the derivative gain P.
 @return double
*/
double MrpPD::getDerivativeGainP() const {
...
*/
Eigen::Vector3d const &MrpPD::getKnownTorquePntB_B() const {
...
*/
void MrpPD::setProportionalGainK(double K) {
    this->algorithm.setProportionalGainK(K);
}

/*! Getter method for the proportional gain K.
 @return double
*/
double MrpPD::getProportionalGainK() const {
    return this->algorithm.getProportionalGainK();
}
