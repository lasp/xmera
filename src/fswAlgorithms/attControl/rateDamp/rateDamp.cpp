...
void RateDamp::updateState(uint64_t currentSimNanos) {
    ...
    if (this->attNavInMsg.isWritten()) { attNavInBuffer = this->attNavInMsg(); }
...
/*! Set the module rate feedback gain
    ...
    @return void
    */
void RateDamp::setRateGain(double p) {
    this->algorithm.setRateGain(p);
}
...
double RateDamp::getRateGain() const {
    return this->algorithm.getRateGain();
}
