...
class KalmanFilterBase {
   public:
    ...
    void updateState(const uint64_t currentSimNanos,
        ...
        for (const auto& measurement : measurements) {
            ...
            if (measurement->getTimeTag() >= this->previousFilterTimeTag && measurement->getValidity()) {
                ...
                //                measurement.setPostFitResiduals(measurement.getObservation() -
                //                measurement.model(this->state)); this->measurements[index] = measurement;
            }
        }
    ...
    const StateType& getState() const { return static_cast<const Derived*>(this)->getStateImpl(); }

    void setState(const StateType& s) { static_cast<Derived*>(this)->setStateImpl(s); }

    void setDynamicsModel(std::function<StateType(double time, const StateType& state)> f) { fx = std::move(f); }
    ...
    void orderMeasurementsChronologically(std::array<std::unique_ptr<IMeasurement<StateDim>>, 5>& measurements) {
        std::sort(measurements.begin(), measurements.end(), [](const auto& a, const auto& b) {
