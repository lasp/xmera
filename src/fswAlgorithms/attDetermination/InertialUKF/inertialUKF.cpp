...
void InertialUKF::updateState(uint64_t callTime) {
    ...
    if (this->firstPassComplete == 0) {
        ...
        if (this->firstPassComplete == 0) { return; }
    ...
    for (i = 0; i < this->STDatasStruct.numST; i++) {
        ...
        if (newTimeTag >= this->timeTag && isFresh) {
            trackerValid = 1;
            if ((newTimeTag - this->timeTag) > this->maxTimeJump && this->maxTimeJump > 0) {
    ...
    if (trackerValid < 1) {
        ...
        inertialUKFAggGyrData(this, this->timeTagOut, newTimeTag, &gyrBuffer);
        /*! - Propagate the attitude quaternion with the aggregate rotation*/
        addMRP(this->sigma_BNOut, this->aggSigma_b2b1, sigma_BNSum);
        ...
        /*! - Rate estimate in this case is simply the low-pass filtered
         ...
         solution*/
        for (i = 0; i < 3; i++) { this->omega_BN_BOut[i] = this->gyroFilt[i].currentState; }
...
void inertialStateProp(InertialUKF* configData, double* stateInOut, double dt) {
    ...
    for (i = 0; i < configData->rwConfigParams.numRW; i++) {
        if (configData->speedDt == 0.0) { continue; }
...
int inertialUKFTimeUpdate(InertialUKF* configData, double updateTime) {
    ...
    ukfQRDJustR(
        AT,
        (int32_t) (2 * configData->countHalfSPs + configData->numStates),
        (int32_t) configData->countHalfSPs,
        rAT
    );
...
void inertialUKFAggGyrData(InertialUKF* configData, double prevTime, double propTime, AccDataMsgPayload* gyrData) {
    ...
    while (minFutTime > prevTime && i < MAX_ACC_BUF_PKT) {
        ...
        for (j = 0; j < 3; j++) { lowPassFilterSignal(omeg_BN_B[j], &(configData->gyroFilt[j])); }
    }
...
int inertialUKFMeasUpdate(InertialUKF* configData, int currentST) {
    ...
    badUpdate += ukfCholDecomp(
        configData->STDatasStruct.STMessages[currentST].noise,
