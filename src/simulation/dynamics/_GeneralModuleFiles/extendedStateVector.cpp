...
void ExtendedStateVector::modify(std::function<void(const size_t&, const std::string&, Eigen::MatrixXd&)> functor) {
    for (auto&& [extendedStateId, stateMatrix] : *this) {
        const auto& [dynObjIndex, stateName] = extendedStateId;
        functor(dynObjIndex, stateName, stateMatrix);
    }
...
ExtendedStateVector ExtendedStateVector::fromStateData(const std::vector<DynamicObject*>& dynPtrs,
