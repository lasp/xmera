...
class PrescribedRotation1DOF : public SysModel {
   ...
   private:
    ...
    bool isInFourthSmoothedSegment(
        double time) const;  //!< Method for determining if the current time is within the fourth smoothing segment for
                             //!< the smoothed bang-coast-bang option
    ...
    void computeFirstBangSegment(
        double time);  //!< Method for computing the first bang segment scalar rotational states
    ...
    void
    computeRotationComplete();  //!< Method for computing the scalar rotational states when the rotation is complete

    void writeOutputMessages(uint64_t currentSimNanos);  //!< Method for writing the module output messages and
                                                         //!< computing the output message data
    Eigen::Vector3d computeSigma_FM();  //!< Method for computing the current spinning body MRP attitude relative to the
                                        //!< mount frame: sigma_FM
