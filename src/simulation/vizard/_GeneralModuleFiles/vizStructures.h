...
//@endcond
{
    ...
    std::string bodyTarget;   //!< Name of body camera should point to (default to first celestial body in messages).
                              //!< This is a setting for body targeting mode.
    int setView = 0;  //!< 0 -> Nadir, 1 -> Orbit Normal, 2 -> Along Track (default to nadir). This is a setting for
                      //!< body targeting mode.
...
//@endcond
{
    ...
    std::vector<std::string>
        simBodiesToModify;  //!< Which bodies in scene to replace with this model, use ``ALL_SPACECRAFT`` to apply
                            //!< custom model to all spacecraft in simulation
    ...
    int shader = -1;  //!< (Optional) Value of -1 to use viz default, 0 for Unity Specular Standard Shader, 1 for Unity
                      //!< Standard Shader
    std::vector<int> color;  //!< [] Send desired RGBA as values between 0 and 255, default is gray, and will be applied
                             //!< to the albedo color setting
} CustomModel;
...
//@endcond
{
    ...
    std::vector<double> fieldOfView;  //!< [rad] edgle-to-edge field of view, single positive value means a conical
    ...
    std::vector<int> color;  //!< [] (optional) RGBA as values between 0 and 255, multiple colors can be populated in
                             //!< this field and will be assigned to the additional mode (Modes 0 and 1 will use the 0th
                             //!< color, Mode 2 will use the color indexed to 1, etc.  If 2 colors are provided, then
                             //!< the vector should have size 8 (2x4 color channels)
...
//@endcond
{
    ...
    double gammaSaturation = -1;  //!< (Optional) Desired gamma saturation of the light lens, 0 to match light
                                  //!< color, 1.0 for pure white, default is 0.8
...
//@endcond
{
    ...
    double currentValue =
        0;                //!< [] current absolute value of the storage device, set from the input message if connected
    ...
    std::vector<int>
        color;  //!< [] (Optional) Send desired RGBA as values between 0 and 255, multiple colors can be populated in
                //!< this field and will be used to color the bar graph between thresholds (i.e. the first color will be
                //!< used between values of 0 and threshold 1, the second color will be used between threshold 1 and
                //!< 2,..., the last color will be used between threshold n and the maxValue
...
//@endcond
{
    ...
    int32_t orbitLinesOn =
        0;  //!< Toggle to show osculating orbit lines, Value of 0 (protobuffer default) to use viz default, -1 for
            //!< false, 1 for relative to parent body, 2 for relative to chief spacecraft body
    int32_t trueTrajectoryLinesOn =
        0;  //!< Toggle to show true orbit lines, Value of 0 (protobuffer default) to use viz default, -1 for false, 1
            //!< to use inertial positions, 2 for relative to chief spacecraft body
    ...
    std::vector<PointLine>
        pointLineList;                    //!< vector of lines pointing from the spacecraft towards another scene object
    ...
    std::string defaultSpacecraftSprite =
        "";  //!< Set sprite for ALL spacecraft through shape name and optional int RGB color values [0,255] Possible
             //!< settings: "CIRCLE","SQUARE", "STAR", "TRIANGLE" or "bskSat" for a 2D spacecraft sprite of the bskSat
             //!< shape
    ...
    double defaultThrusterPlumeLifeScalar =
        1.0;  //!< Value of 1.0 or 0.0 to use viz default, values between 0 and 1 will decrease the length of all
              //!< plumes, >1 will increase lengths of all plumes
    ...
    std::string mainCameraTarget = "";  //!< If valid spacecraft or celestial body name is provided, the main camera
                                        //!< will be targeted at that body at start
    ...
    double spacecraftHelioViewSizeMultiplier =
        -1;  //!< Control the display size of spacecraft in the Solar System View, values greater than 0, use negative
    ...
    int32_t planetViewToHelioViewBoundaryMultiplier =
        0;  //!< Multiplier x (10000 * current planet local scale) at which the planet view transitions to the solar
    ...
    int32_t attenuateSunLightWithDistance =
        0;  //!< Toggle to reduce brightness of sun lighting with the square of the distance from the sun. Value of 0
            //!< (protobuffer default) to use viz default, -1 for false, 1 for true.
    int showLightLabels = 0;  //!< Toggle to label spacecraft light elements, Value of 0 (protobuffer default) to use
...
typedef struct {
    ...
    std::string modelDictionaryKey;  //!< (optional) Vizard model key to use.  If set, it over-rides the model selected
                                     //!< by the name
} GravBodyInfo;
