...
void VizInterface::WriteProtobuffer(uint64_t currentSimNanos) {
...

    {
        ...
        if (this->broadcastStream) {
            ...
            if (sendStatus == -1) { bskLogger.bskLog(BSK_ERROR, "Broadcast header did not send to socket."); }
            sendStatus = zmq_send(this->publisher_socket, serialized_message, byteCount, 0);
            if (sendStatus == -1) { bskLogger.bskLog(BSK_ERROR, "Broadcast protobuffer did not send to socket."); }
        }
         ...
         * visual capabilities during OpNav */
        /*!--OpNavMode set to 2 is a faster mode in which the viz only steps forward to the BSK time step if an image is
         * requested. This is a faster run but nothing can be visualized post-run */
        ...
        if (this->opNavMode == 1 || (this->opNavMode == 2 && opNavModeStatus) || this->liveStream) {
            // Receive pong
