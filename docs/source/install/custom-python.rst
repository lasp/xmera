...
The following instructions are guidelines on how to run Xmera with a
computer that is not using a system installed version of Python.

-  Xmera must be built and run with the same Python binary.
...
-  If you use a virtual environment, but sure to build Xmera in that virtual environment for the simulation
