...
- Doing a clean build on Windows appeared to complete, but when running python simulation scripts,
  errors came up about not finding Xmera packages.  The python version number checking on Windows
  had an issue that is now corrected in the current build.
- ``swig`` 4.2 was causing run-time errors with Xmera.  The latest version of Xmera now added
  support for this version of swig.
- Xmera no longer builds on Windows with the ``opNav`` flag turned on.  The ``opencv`` related
...
- Doing a clean Xmera build with `opNav` flag on fails building openCV.  The conan
...
- Building Xmera with ``opNav`` mode was no longer working as a conan package dependency issue came up.
...
- updated ``examples/OpNavScenarios/BSK_OpNav.py`` path to the Vizard binary. If you are calling
  Vizard 2.1+ from the python script, the Vizard binary name changed from ``Xmera Vizard` to
  simply ``Vizard``.
...
- ``conan`` version 1.44 is causing build issues because the way the source and build folders are set has
  changed.  This is fixed in current version of Xmera.
- building Xmera from scratch with ``opNav`` turned on was failing because ``conan`` was not able to
...
- In Xcode, when editing ``vizInterface.c/h`` files, the protobuffer library is not properly found when opNav is included.
  The code compiles, but auto-completion etc. doesn't work in that module.
- ``conan`` changed the default repos in version 1.40 onwards which broke the Xmera installation.
...
- Over the summer of 2020 the ``conan`` repo wasn't able to install ``opencv`` anymore.  The current build
  fixes this issue and lets Xmera be compiled with the ``opNav`` flag again.
...
- In Xcode, when editing ``vizInterface.c/h`` files, the protobuffer library is not properly found when opNav is included.
  The code compiles, but auto-completion etc. doesn't work in that module.
- On Windows, using Python 3.8 or higher resulted in Xmera paths not being recognized due to a chance with how
...
- The new build system provides many speed improvements in doing a clean or partial build, but some small changes are
  ...
  - In BSK python simulation scripts, BSK modules should be included using the indirect method.  Thus::
        ...
        from Xmera.fswAlgorithms.cModuleTemplate import cModuleTemplate

    becomes::

        from Xmera.fswAlgorithms import cModuleTemplate

  - The ``pyswice`` package is now imported from ``topLevelModule``.  Thus::

        from Xmera import pyswice

    becomes::

        from Xmera.topLevelModules import pyswice

  - The support file ``pyswice_ck_utilities.py`` has become a regular suppoort file in ``src/utiliites``.  Thus,
    it is imported using::

        import Xmera.pyswice.pyswice_ck_utilities

  - Similarly, ``pyswice_spk_utilities.py`` has moved to the utilities folder. To include ``spkRead`` function replace::

        from Xmera.pyswice.pyswice_spk_utilities import spkRead

    with::

        from Xmera.utilities.pyswice_spk_utilities import spkRead

  - To include ``loadGravFromFileToList`` function replace::

        from Xmera.simulation.gravityEffector.gravCoeffOps import loadGravFromFileToList
...
- If you have written custom BSK modules outside of the BSK distribution, the swig ``*.i`` files and some code files
  ...
  - To include the ``swig_common_model.i`` file, replace::
    ...
    with::
        ...
        %pythoncode %{
        from Xmera.simulation.swig_common_model import *
        %}
...
- On Windows Xmera didn't compile due to missing math ``#define`` delaration in ``geodeticConversion.cpp/h``.
...
- WINDOWS ONLY: Windows users cannot currently run pytest directly on Xmera ``src/`` directory (there will be non-resolved python path issues that will result in erroneous ImportErrors). Instead, to verify proper installation of Xmera, windows users must enter the specific subdirectory they are attempting to test, only then to run pytest. This should result in appropriate behavior.  Right now there is no known solution to this issue.
- ``BSK_PRINT`` has been replaced within Xmera modules using ``bskLogging`` (for C++) and ``_bskLog`` (for C).
...
Xmera ``src/`` directory (there will be non-resolved python path
issues that will result in erroneous ImportErrors). Instead, to verify
proper installation of Xmera, windows users must enter the specific
...
compatible with our current Xmera software. Be sure to install swig
version 3.0.12.
...
This version of Xmera no longer support the ASIO module that
...
Version 4.x.x and higher of pytest works again with Xmera. You are
...
command line directly. See the platform specific Xmera installation
instructions.
