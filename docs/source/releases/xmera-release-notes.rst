...
- Fixed a bug where the legacy variable logging API would either, not log at all or log at a rate different to the
  requested rate.
- Fixed a python version checking bug that prevented Xmera from compiling on Windows
...
- Updated versioning to better follow the `semantic versioning <https://semver.org>`_ standard, in the format
  ...
  automatically increment the patch number. This allows users to reference/require specific versions of Xmera
...
- Changed how C modules are wrapped as C++ classes. This makes handling C modules the same as C++ modules,
  ...
  To convert prior script to use the new syntax, see :ref:`xmeraPrinciples-2` for the simple new
...
- Implemented new syntax for variable logging. See :ref:`xmeraPrinciples-6`.
- Added :ref:`scenarioMomentumDumpingZeroNetForce` scenario to illustrate momentum dumping without exerting a net force
  on the spacecraft.
- Xmera minimum Python version is now formally 3.8.x (checked by build files). Previously, it was indicated to be
...
- Added a new scenario :ref:`scenarioHohmann` that performs a Hohmann transfer with attitude mode changes.
  The basic attitude flight modes are implemented using the Xmera event system.
...
- Fixed examples and tests to run even when Xmera is built with ``--vizInterface False``.
...
-----------------------------
- Created new way to define Python modules by inheriting from ``Xmera.architecture.sysModel.SysModel``.
  See pyModules for details.
- Added the ability to integrate the ODE's of two or more Xmera modules that are ``DynamicObject`` class
  member at the same time.  See :ref:`xmeraPrinciples-9`
...
- The Xmera project C++ version is advanced from C++11 to C++17
- Disabled the following build options in the conan included OpenCV dependency; with_ffmpeg video frame encoding lib,
  ...
  format, with_quirc QR code lib. Users that have Xmera control the build of these modules through the External
...
- Added a deprecation system for Xmera. For developers, see :ref:`deprecatingCode`.
...
------------------------------
- Fixed ``CMake/conan`` case sensitivty issue when compiling Xmera with ``opNav`` flag set to ``True`` on Linux platforms
...
- Updated Xmera install documentation to discuss accessing source code from GitHub.com
...
- Added documentation in :ref:`xmeraPrinciples-4` on how to read the current message values
- Highlighted the challege of setting up a ``recorder`` on a re-directed message in :ref:`xmeraPrinciples-7`
...
- Fix an issue in in :ref:`magneticFieldWMM` where a fixed width array holding a file path would result in a cutoff
  path when xmera is located in a directory path of greater than 100 characters.
...
- The ``linearAlgebra``, ``rigidBodyKinematics``, ``orbitalMotion`` were mistakenly exposed as part of the
  ...
  ``Xmera.utilities.orbitalMotion``, ``Xmera.architecture.linearAlgebra``, and
  ``Xmera.architecture.rigidBodyKinematics``.
...
- Updated :ref:`pullCloneBSK` to ask the user to first install ``lfs`` before pulling a copy
  of the Xmera repo due to some large files being stored in the GitHub large file storage
  system.
- Updated :ref:`scenarioGroundLocationImaging` to properly save off the ground location
  information for Vizard
- Added a new helper function to convert C arrays to ``Eigen::MRPd`` and vice-versa inside ``eigenSupport``.
...
- Updated :ref:`cppModules-4` to explain how now the swig interface to vectors of input/output messages
  are now auto-generated when making Xmera project
...
- Added experimental support to build Xmera on Linux with a computer using an ARM processor.
...
- Updated documentation to discuss downloading Xmera from GitHub
...
- revised how the build system swig's all the message objects.  This leads to compile time improvements across all
  ...
  the memory requirements when compiling. Note: The ``xmera.architecture.cMsgCInterfacePy`` content is
...
- added new training videos to :ref:`scenario_BasicOrbit`, :ref:`scenario_FeedbackRW` to discuss how to
  create class based Xmera simulations
...
- updated Xmera documentation CSS to work with the latest version of ``sphinx`` and ``breathe``
...
- added new training videos to configureBuild, installOptionalPackages, :ref:`scenarioBasicOrbit`,
...
- improved robustness of Xmera installation script
...
- This release provides a new ability to run a single Xmera simulation in a multi-threaded manner.
...
Version 2.0.2
-------------
- new waypoint reference module in :ref:`waypointReference`. It can be used to read an attitude maneuver from a set of waypoints on a text file, likely generated outside Xmera.
...
- changed default behavior of ``python3 conanfile.py`` to automatically compile the Xmera project.  This was
...
- added ``ShowExecutionFigure()`` method to :ref:`SimulationBaseClass` to create a figure illustration the
  execution order.
- added a new :ref:`xmeraPrinciples-2b` web page on how to visualize the BSK process, task and module execution
...
- moved cModuleTemplate and :ref:`cppModuleTemplate` to a common folder ``src/moduleTemplates``.  The
...
- Update Xmera module documentation that shows all input and output message variables, their
  type and explanation
- Cleaned up the Xmera `src` folder layout by moving all Xmera architecture support files
...
- Updated message names to now all comply with the Xmera message naming convention.  See
  :ref:`migrating-module-to-bsk2` for a table of how some message names have changed
...
- Created several instructional pages in the Quick-Start documentation folder.  The examples folder
  ...
  - how to write Xmera python simulation scripts
  - how to write C++, C and Python modules

- Added installation instructions to run Xmera on a computer with the Apple M1 processor
...
- renamed ``fswModuleTemplate()`` to be cModuleTemplate.  This makes this naming consistent with the new :ref:`cppModuleTemplate`.
...
- Support Terminal Progress bar while running a Xmera simulation
- Improved the build system to re-swig the module if the dependencies have changed.
  This avoids having to do a clean build or manually deleting the swing python files from within ``dist3/Xmera``.
...
- updated ``eigenSupport.h`` to add new methods ``eigenMRPd2Vector3d()`` and ``eigenC2MRP()``
...
- Improved the build process for Linux such that ``vizInterface`` and ``opNav`` related modules are available
  again in Xmera python scripts.  Thus Linux users can use 1.8.x onwards and still use these enhanced features.
...
- hot-fix of an issue compiling Xmera on Windows.  A ``#define _USE_MATH_DEFINES`` was missing that
...
- updated documentation of cModuleTemplate to show how to make much simpler lists of module messages
...
- Fixed a transformation issue in ``eigenSupport.cpp`` where ``cArray2EigenMatrix3d()`` has to deal with
...
- Many new Xmera scenarios illustration interfacing with :ref:`Vizard <Vizard>` to simulate opNav cases:
...
- Updated cModuleTemplate to include a message I/O figure and move it's message definition to ``simMessages``
...
- Updated cModuleTemplate documentation to show how to add equation numbers, cite equations, do bold math variables and cite a figure caption.
...
- New spacecraft formation flying scenario :ref:`scenarioFormationBasic` where 3 satellites are flying 10m apart in a lead-follower configuration.  Each has a different number of RWs.  This scenario is a nice script to demonstrate the new multi-spacecraft support in Vizard.

**Version 1.4.2**

- added link to Xmera facebook page to Sphinx-based documentation
...
- Updated the :ref:`aboutBSK` page to include Xmera highlights
...
- Added a new Xmera logging system called bskLogging.  This allows modules to print information with a variable verbosity level
...
- It is no longer necessary to call sim.TotalSim.terminateSimulation() at the beginning of Xmera scripts. This call has been moved to the SimBaseClass constructor and removed from scripts in the repository.
...
- New Sphinx/Breathe based BSK documentation system!  All documentation is still stored in the ``xmera/docs`` folder.  The new system provides much better directory structure to access the BSK modules, and has a cleaner way to list the tutorial examples.
...
Added the ability to live stream the Xmera simulation data to Vizard!
...
Added the ability to specify Vizard settings from Xmera
``vizInterface`` module settings. This way Xmera simulations can set
...
ADDED PYTHON 3 SUPPORT! This is a major step for Xmera. Python 2
...
link Xmera and Vizard with a TCP connection. This is ground work for
...
Removed the BOOST library from Xmera as it is no longer needed. This
...
added a PPTX support file that explains the core Xmera architecture.
...
Providing new support functions to enable the upcoming Vizard Xmera
Visualization.
...
to be installed and configured. See the updated Xmera installation
...
Updated the macOS Xmera installation instructions to make them easier
...
This provides for a cleaner and easier to maintain Xmera
installation.
...
steps for setting up and installing Xmera on machine with a fresh
copy of Windows 10.
...
First open beta release of Xmera
...
the Xmera ``pytest`` command is called. There is also some
