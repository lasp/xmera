...
#. I'm having trouble running ``pytest`` on Windows from within the ``src`` directory.
    ...
    for 8 threads, for example.  See ``installOptionalPackages`` for more information on installing ``pytest-xdist``.

#. I have upgraded to Xmera version 1.8.0 or newer and I'm following the revised build process.  However,
