...
- check that required input messages are connected.  If a required input message is not connected when ``reset()`` is called, then log an error message.
...
.. warning::
    ...
    It is critical that each module zeros the content of the output messages on each update cycle.  This way we are not writing stale or uninitialized data to a message.  When reading a message Xmera assumes that each message content has been either zero'd or written to.
