...
Parent Class
------------
Every Xmera module is a sub-class of :ref:`sys_model`.  This parent class provides common modules variables such as ``modelTag`` and ``moduleID``.

Module Class Name
-----------------
The Xmera module class name should be descriptive and unique.  Its spelling has to be upper camel case and thus start with a capital letter.
...
.. code-block:: cpp
    ...
    /*! @brief basic Xmera C++ module class */
...
- All ``include`` statements should be made relative to ``xmera/src``

- The ``sys_model.h`` file must be imported as Xmera modules are a subclass of :ref:`sys_model`.

- All the message payload definition files need to be included.  In the above example there is only one.

- Include the Xmera ``messaging.h`` file to have access to creating message objects

- Include the ``bskLogging.h`` file to use the Xmera logging functions.

Be sure to add descriptions to both the module and to the class variables used.

Required Module Methods
-----------------------
Each module should define the ``reset()`` method that is called when initializing the Xmera simulation, and the ``updateState()`` method which is called every time the task to which the module is added is updated.
...
Finally, the ``bskLogger`` variable is defined to allow for Xmera message logging with variable verbosity.  See :ref:`scenarioBskLog` for an example of how to set the logging verbosity.
